(function () {
  ...
  function loadState() {
    try {
      ...
      if (Array.isArray(parsed.savedQuestions)) {
        state.savedQuestions = Array.from(new Set(parsed.savedQuestions
          .map((value) => Number.parseInt(value, 10))
          .filter((value) => Number.isInteger(value) && value >= 0 && value < questions.length)));
      }

      if (typeof parsed.savedOpen === 'boolean') {
        state.savedOpen = parsed.savedOpen;
      }
    } catch {
  ...
  function toggleSaved(force) {
    state.savedOpen = typeof force === 'boolean' ? force : !state.savedOpen;
    render();
  }

  function toggleSaveCurrentQuestion() {
    const questionIndex = currentQuestionIndex();
    const existingIndex = state.savedQuestions.indexOf(questionIndex);
    if (existingIndex >= 0) {
      state.savedQuestions.splice(existingIndex, 1);
    } else {
      state.savedQuestions.push(questionIndex);
    }
    render();
  }
  ...
  function renderSavedPanel() {
    const savedEntries = state.savedQuestions
      .map((questionIndex) => ({ questionIndex, question: questions[questionIndex] }))
      .filter((entry) => entry.question);

    els.savedCount.textContent = `${savedEntries.length} pyetje`;
    els.savedButton.textContent = state.savedOpen ? 'Mbyll pyetjet e ruajtura' : 'Pyetjet e ruajtura';
    els.savedPanel.hidden = !state.savedOpen;
    ...
    if (!savedEntries.length) {
      const empty = document.createElement('div');
      empty.className = 'review-empty';
      empty.textContent = 'S’ke ruajtur ende pyetje.';
    ...
    savedEntries.forEach((entry) => {
      ...
      item.innerHTML = `
        <span class="review-number">Pyetja ${entry.questionIndex + 1}</span>
        <span class="review-question">${escapeHtml(entry.question.question)}</span>
      `;
      item.addEventListener('click', () => {
        animateTap(item);
        goToQuestionByIndex(entry.questionIndex);
  ...
  function render() {
    ...
    els.savedButton.disabled = false;
    ...
    renderReviewPanel();
    renderSavedPanel();
    els.saveButton.textContent = state.savedQuestions.includes(currentQuestionIndex()) ? 'Hiqe nga ruajtja' : 'Ruaj pyetjen';
    saveState();
  }
  ...
  els.saveButton.addEventListener('click', (event) => {
    animateTap(event.currentTarget);
    toggleSaveCurrentQuestion();
  });

  els.savedButton.addEventListener('click', (event) => {
    animateTap(event.currentTarget);
    toggleSaved();
  });
